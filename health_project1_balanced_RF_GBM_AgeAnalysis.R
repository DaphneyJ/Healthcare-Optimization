...
rf3_confusion
tn_rf <- rf3_confusion$table[1, 1]  # True Negative
fn_rf <- rf3_confusion$table[2, 1]  # False Negative
fp_rf <- rf3_confusion$table[1, 2]  # False Positive
tp_rf <- rf3_confusion$table[2, 2]  # True Positive
...
#################### Part 2: Business Adaptation #################

################# At Risk w/ Exercise as Outcome ###############

# # Split data into Training and Testing sets (70% train, 30% test)
# set.seed(123)
# split <- sample.split(at_risk$ExerciseFrequency, SplitRatio = 0.7)
# train_data <- subset(at_risk, split == TRUE)
# test_data <- subset(at_risk, split == FALSE)
# 
# #Verify Distribution in sets
# table(train_data$ExerciseFrequency)/nrow(train_data) *100
# table(test_data$ExerciseFrequency)/nrow(test_data) *100
# 
# #LASSO
# x_train <- model.matrix(ExerciseFrequency ~ . - 1, data = train_data)  # -1 removes intercept to prevent duplication
# x_test <- model.matrix(ExerciseFrequency ~ . - 1, data = test_data)
# y_train <- train_data$ExerciseFrequency
# y_test <- test_data$ExerciseFrequency
# 
# # LASSO Multinomial Models
# lasso_model <- cv.glmnet(x_train, y_train, family = "multinomial", alpha = 1) #standardized by default
# 
...
# 
# #View Lasso selected variables
# coefficients_atRisk_exercise <- coef(lasso_model, s = "lambda.min") #Health: -systolic_stg1, +BMI_overweight
...
###Boosting## Only FYSA - Logistic Multinomial works better
boost_20s <- gbm(Outcome ~., data = train_data, 
                   distribution = "multinomial", n.trees = 1000, shrinkage = .01, 
                   interaction.depth = 3, cv.folds = 10)
test_boost_20s <- ifelse(predict(boost_20s, newdata = test_data, n.trees = 1000, 
                                   type = "response") < 0.5, 0, 1)
test_boost_20s <- drop(test_boost_20s)
y_matrix = model.matrix(~ y_test + 0) #converting test outcomes to useable format
y_array = as.array(y_matrix)
test_error_boost_20s <- mean(test_boost_20s != y_array)
test_error_boost_20s

#parameter tuning for optimal trees
optimal_trees <- gbm.perf(boost_20s, method = 'cv')
optimal_trees
#optimal number of trees is 507

summary(boost_20s)

test_model_optimal_trees_20s <- ifelse(predict(boost_20s, newdata = test_data, n.trees = optimal_trees, 
                                           type = "response") < 0.5, 0, 1)
test_model_optimal_trees_20s <- drop(test_model_optimal_trees_20s)
test_error_optimal_trees_20s <- mean(test_model_optimal_trees_20s != y_array)
test_error_optimal_trees_20s

boost_accuracy_20s <- 1- test_error_optimal_trees_20s
boost_accuracy_20s

confusion_boosting_20s <- confusionMatrix(factor(test_model_optimal_trees_20s), factor(y_array))
tn_gbm <- confusion_boosting_20s$table[1, 1]  # True Negative
fn_gbm <- confusion_boosting_20s$table[2, 1]  # False Negative
fp_gbm <- confusion_boosting_20s$table[1, 2]  # False Positive
tp_gbm <- confusion_boosting_20s$table[2, 2]  # True Positive
precision_gbm <- tp_gbm / (tp_gbm + fp_gbm)
recall_gbm <- tp_gbm / (tp_gbm + fn_gbm)
f1_score_gbm_20s <- 2 * (precision_gbm * recall_gbm) / (precision_gbm + recall_gbm)
...
##Boosting## Only FYSA - Logistic Multinomial works better
boost_30s <- gbm(Outcome ~., data = train_data, 
                 distribution = "multinomial", n.trees = 1000, shrinkage = .01, 
                 interaction.depth = 3, cv.folds = 10)
test_boost_30s <- ifelse(predict(boost_30s, newdata = test_data, n.trees = 1000, 
                                 type = "response") < 0.5, 0, 1)
test_boost_30s <- drop(test_boost_30s)
y_matrix = model.matrix(~ y_test + 0) #converting test outcomes to useable format
y_array = as.array(y_matrix)
test_error_boost_30s <- mean(test_boost_30s != y_array)
test_error_boost_30s

#parameter tuning for optimal trees
optimal_trees <- gbm.perf(boost_30s, method = 'cv')
optimal_trees
#optimal number of trees is 507

summary(boost_30s)

test_model_optimal_trees_30s <- ifelse(predict(boost_30s, newdata = test_data, n.trees = optimal_trees, 
                                               type = "response") < 0.5, 0, 1)
test_model_optimal_trees_30s <- drop(test_model_optimal_trees_30s)
test_error_optimal_trees_30s <- mean(test_model_optimal_trees_30s != y_array)
test_error_optimal_trees_30s

boost_accuracy_30s <- 1- test_error_optimal_trees_30s
boost_accuracy_30s

confusion_boosting_30s <- confusionMatrix(factor(test_model_optimal_trees_30s), factor(y_array))
tn_gbm <- confusion_boosting_30s$table[1, 1]  # True Negative
fn_gbm <- confusion_boosting_30s$table[2, 1]  # False Negative
fp_gbm <- confusion_boosting_30s$table[1, 2]  # False Positive
tp_gbm <- confusion_boosting_30s$table[2, 2]  # True Positive
precision_gbm <- tp_gbm / (tp_gbm + fp_gbm)
recall_gbm <- tp_gbm / (tp_gbm + fn_gbm)
f1_score_gbm_30s <- 2 * (precision_gbm * recall_gbm) / (precision_gbm + recall_gbm)
